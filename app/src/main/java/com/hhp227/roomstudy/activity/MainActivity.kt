    ...
    private val activityResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        result.data?.let { intent ->
            when (result.resultCode) {
                RESULT_OK -> mainViewModel.insertMemo(
                    MemoDto(
                        title = intent.getStringExtra(TITLE_MEMO) ?: "",
                        text = intent.getStringExtra(TEXT_MEMO) ?: ""
                    )
                )
                RESULT_REMOVE -> mainViewModel.selectMemo(intent.getIntExtra(ID_MEMO, -1)).value?.also { memoDto ->
                    Toast.makeText(applicationContext, "테스트: $memoDto", Toast.LENGTH_LONG).show()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView<ActivityMainBinding>(this, R.layout.activity_main).apply {
            ...
            recyclerView.apply {
                adapter = MemoAdapter().apply {
                    ...
                    setOnItemClickListener { _, p ->
                        Intent(baseContext, WriteActivity::class.java).putExtra(REQUEST_CODE, REQUEST_CODE_SET)
            ...
            fab.setOnClickListener {
                activityResultLauncher.launch(Intent(baseContext, WriteActivity::class.java).putExtra(REQUEST_CODE, REQUEST_CODE_ADD))
            }
        }
    }
